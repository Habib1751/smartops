...
export async function GET(request: NextRequest) {
  try {
    const externalUrl = `${EXTERNAL_API_BASE}/api/management/reports/dashboard`;
    ...
    console.log('✅ Successfully fetched dashboard report from external API');
    ...
    console.error('❌ Error proxying dashboard report request:', error.message);
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to fetch dashboard report from external API',
