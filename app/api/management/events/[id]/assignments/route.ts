...
export async function GET(
  ...
  try {
    ...
    const externalUrl = `${EXTERNAL_API_BASE}/api/management/events/${eventId}/assignments`;
    ...
    console.log('✅ Successfully fetched event assignments from external API');
    ...
    console.error('❌ Error proxying event assignments request:', error.message);
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to fetch event assignments from external API',
